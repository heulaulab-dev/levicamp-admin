...
export const useExport = create<ExportState>((set) => {
	...
	const makeExportRequest = async <T>(endpoint: string): Promise<T | null> => {
		...
		try {
			...
			} else if (endpoint.includes('daily-guest-count')) {
				filename = `levicamp_daily_guest_count_export_${today}.xlsx`;
			}
	...
	return {
		...
		// Export daily guest count data
		exportDailyGuestCount: async (startDate: string, endDate: string) => {
			return makeExportRequest<string>(
				`/bookings/export/daily-guest-count?start_date=${startDate}&end_date=${endDate}`,
			);
		},
	};
});